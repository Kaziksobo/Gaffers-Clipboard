...
class App(ctk.CTk):
    ...
    def _cleanup_screenshots(self, max_files: int = 5) -> None:
        """Prune older stats screenshots to keep the capture directory manageable.

        Retains only the newest ``max_files`` screenshots matching the expected
        naming pattern and silently skips cleanup when the directory is absent.

        Args:
            max_files (int, optional): The maximum number of recent screenshots to keep.
                Any additional older files beyond this count will be deleted. Defaults to 5.
        """
        screenshots_dir = App.PROJECT_ROOT / "screenshots"
        if not screenshots_dir.exists():
            return
    ...
    def buffer_match_overview(self, overview_data: dict[str, Any]) -> None:
        """Stage the high-level match overview data in the in-memory session buffer.

        Accepts a single overview payload and replaces any previously buffered 
        overview so that subsequent save operations act on the latest match context.

        Args:
            overview_data (dict[str, Any]): The raw match overview details captured 
                for the current match, such as scoreline, venue, and date.
        """
