...
class PlayerStatsFrame(
    ...
    def _on_next_outfield_player_button_press(self) -> None:
        ...
        """
        if not self._collect_data_unless_skipped():
            return
        try:
    ...
    def _on_done_button_press(self) -> None:
        ...
        Unless skip-save is enabled, this action collects and buffers the
        current player's stats before instructing the controller to save all
        buffered match records and route to the confirmation frame.
        """
        if not self._collect_data_unless_skipped():
            return
        try:
    ...
    def _collect_data_unless_skipped(self) -> bool:
        ...
        if skip_var is not None and bool(skip_var.get()):
            logger.info("Skipping save for current outfield player by user request.")
    ...
    def _validate_maximums(self) -> bool:
        ...
        return all(
            self.validate_stat_max(self.ui_data, key, label, max_value=max_val)
